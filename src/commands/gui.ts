...
import { type ButtonInteraction, type CommandInteraction, PartialGroupDMChannel } from "discord.js";
...
export class Command {
  ...
  async guiShow(interaction: CommandInteraction): Promise<void> {
    const cmd = await musicPlayerManager.parseCommand(interaction);
    if (!cmd || !interaction.channel || interaction.channel instanceof PartialGroupDMChannel) {
      return;
    }
