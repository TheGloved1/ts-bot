...
export class Command {
  ...
  async spotify(
    ...
    if (result === null) {
      await interaction.followUp("The Spotify url you provided appears to be invalid, make sure that you have provided a valid url for Spotify");
      return;
    }

    const videos = await Promise.all(result.map((track) => YouTube.searchOne(`${track.name} by ${track.artist}`)));
    ...
    embed.setDescription(`Enqueued  **${String(tracks.length)}** songs from spotify playlist`);
